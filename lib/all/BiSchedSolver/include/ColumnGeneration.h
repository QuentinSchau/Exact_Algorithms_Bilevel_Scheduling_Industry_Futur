...
private:
    ...
    struct InfoColumn {
        ...
        std::bitset<MAX_NUMBER_JOB> encodingColumn; // Bitset representation of the column for efficient storage and comparison
        ...
        inline InfoColumn(std::vector<unsigned int> &&col, unsigned int indexOfVariable, unsigned int nbUnfeasible, double costOfSchedule, TYPE_COLUMN typeColumn) :
                column(std::move(col)), // Move the vector of column into the struct
                encodingColumn(), // Initialize bitset with default values (all set to false)
