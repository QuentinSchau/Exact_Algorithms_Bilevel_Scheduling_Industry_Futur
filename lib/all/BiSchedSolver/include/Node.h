...
#define DOT_MEMO_DOMINATED "[style=filled,fillcolor=greenyellow]"
#endif
...
private:
    ...
    std::bitset<MAX_NUMBER_JOB> listGroupedJobRemoved;
    ...
    std::bitset<MAX_NUMBER_JOB> encodingRemoveDecision; // encoding vector that represents the removed jobs
    std::vector<std::bitset<MAX_NUMBER_JOB>> encodingSelectedJobOnMachines; // encoding vector that represents the selected jobs on each machine
...
//        // change the block structure
    ...
    [[nodiscard]] const std::bitset<MAX_NUMBER_JOB> &getEncodingRemoveDecision() const {
        return encodingRemoveDecision;
    }

    [[nodiscard]] const std::vector<std::bitset<MAX_NUMBER_JOB>> &getEncodingSelectedJobOnMachine() const {
 ...
 */
     ...
     */
    static void setEncodingForJob(unsigned int indexOfJob, std::bitset<MAX_NUMBER_JOB> &encoding) {
